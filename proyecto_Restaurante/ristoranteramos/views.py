...
def new_resena(request):
    if not request.user.is_authenticated:
        messages.warning(request, "Debes registrarte e iniciar sesión para crear una reseña.")
        return redirect('log_in_page')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            nueva_resena = Resena()
            nueva_resena.fecha_creacion = datetime.now()
            nueva_resena.usuario = request.user
            nueva_resena.puntuacion = request.POST.get('puntuacion')
            nueva_resena.comentarios = request.POST.get('comentarios')
            nueva_resena.save()
            return redirect('resenas')
    else:
            form = FormularioResena()

    return render(request, 'anadirResena.html',{'form':form})

def go_resenas(request):
    resenas = Resena.objects.all()
    return render(request, 'verResenas.html', {"resenas": resenas})

@login_required
def go_editar_resenas(request):
    resenas = Resena.objects.filter(usuario=request.user)
    return render(request, 'editarResenas.html', {"resenas": resenas})
...
def eliminar_resena(request, id):
    resena_eliminar = Resena.objects.filter(id=id)
